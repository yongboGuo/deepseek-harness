...
**Net:** dsh SDK is *runtime-replaceable*; the other two are *model-replaceable inside a fixed loop*.
...
| Cancellation / stop | `client.close()`; subprocess killed via stdin-EOF → SIGTERM → SIGKILL ladder; ladder is private to the client | `client.interrupt()` aborts the in-flight turn; `cancel_queued` (capability `interrupt_cancel_queued_v1`) cancels queued + pending | `thread.interrupt()`; `turn.interrupt()` |
...
**Note on parity:** the dsh SDK is structurally similar to a JSON-RPC binding of the Claude Code / Codex patterns, but with a tighter contract — the wire is the *whole session log*, not just deltas. The runtime has a "Model-visible means logged" invariant: anything reaching the model must be reconstructable from the log, and the SDK streams that full log to you.
...
## 5. Configuration, extension, and "what can I swap?"
...
## 8. Tools, code execution, and shells
...
| Built-in tools | whatever your `cordis.yml` mounts; `dsh-base` ships a default set | Read, Write, Edit, MultiEdit, Glob, Grep, NotebookEdit, WebFetch, WebSearch, Bash, BashOutput, KillShell, TodoWrite, Agent, Skill | `exec_command`, `shell_command`, `apply_patch`, `update_plan`, MCP-loaded tools |
...
## 9. Lifecycle, hooks, and observability
...
## 10. Provider, model, and runtime neutrality
...
## 11. Beta / experimental / preview surface

### 11.1 dsh — what is marked experimental or not-yet-stable
...
### 11.2 Claude Agent SDK — what is marked beta / alpha / experimental
...
> is annotated alpha/beta at the API surface.

### 11.3 Codex SDK — what is marked beta / experimental
...
| | dsh SDK | Claude Agent SDK | Codex SDK |
|---|---|---|---|
| Filesystem sandbox | `ctx.sandbox` backend | `sandbox: 'worktree'`; `filesystem.autoAllowDirectories`; `sandbox.network.strictAllowlist` | `sandbox_workspace_write.network_access`; `sandbox: 'danger-full-access'`; per-policy |
...
## 13. Performance, scale, and operations
...
## 16. Cross-cutting risks and gotchas
...
## 17. Authoritative source links (cited as of 2026-08-14)
